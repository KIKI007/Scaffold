...
if __name__ == "__main__":
    # input from rhino
    keys = ["Name", "Bar Radius (m)", "Bar Lengths (m)", "Clamp Gap (m)", "Position (m)", "Orientation (rad)", "Time (s)", "Debug"]
    ...
    if parameters:
        ...
        if input_curves and fixed_lines:
            ...
            optimizer = SMILP_optimizer(stick_model_input.stick_model.file_name)
            optimizer.send_message_flag = False
            optimizer.input_model(stick_model_input)
            if optimizer.solve():
                model = optimizer.result_model
                rs.CurrentLayer("Default")
